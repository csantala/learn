  ...
  //      else {
    ...
    $(".rows").on('keydown', '.rowx', (function(e) {
        ...
        switch(e.which) {
            ...
            case 1920:
                e.preventDefault();
                $('<tr class="rowx"><td class="start"><span data-time="'+ moment().format('X') + '">' + moment().format('h:mm') + '</span>:</td><td><input data-time="'+ moment().format('X') + '" maxlength="300" class="task span9" type="text" /></td></tr>').insertBefore($(this));
                update_inputs();
                var time = $(this).find('span').data("time");
